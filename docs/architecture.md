...
flowchart TD
  ...
  subgraph ResultFlow["ResultPage 내부 흐름 — 광고→코인→부활→교환→다시하기 순서 [v0.4]"]
    ...
    R_RV --> R_EX["⑤ PointExchangeButton 상태 결정\n· balance≥10: 활성\n· balance<10: disabled + 안내문"]
    R_EX --> R5["⑥ 다시하기 버튼 활성화"]
    R1 -->|스킵 · 광고 실패| R_RV
  end
```
...
flowchart TD
  ...
  subgraph GameOverFlow["게임오버 후 ResultPage 흐름"]
    GO1["showRewardAd()\n완시청 → resolve(true)"]
    GO2["randomCoinReward()\n1개30% 2개30% 3개25% 4개10% 5개5%\n샌드박스: 고정 2개"]
    GO3{최고기록 갱신?}
    GO4["addCoins(1, 'record_bonus')"]
    GO5["RevivalButton\nbalance≥5 AND !revivalUsed\n→ addCoins(-5, 'revival')\n→ store.revive()"]
    GO6["PointExchangeButton\nbalance≥10\n→ grantCoinExchange()\n→ addCoins(-10, 'toss_points_exchange')"]
  end
...
ResultPage ──────────────────────────┤
  ...
  └── PointExchangeButton (F5: 10코인 교환)
...
| 6 | F5: PointExchangeButton + grantCoinExchange | F1 | — |
| 7 | Polishing: 잔액 UI + float-up 애니메이션 | F2~F5 | — |

> ⚠️ F1 완료 전에는 F2~F7 구현 불가 (useCoin이 모든 코인 기능의 공통 기반)
