...
- [ ] `useGameEngine.ts` — `gameOver('timeout')` / `gameOver('wrong')` 호출, `gameOverReason` 반환
- [ ] `src/components/game/GameOverOverlay.tsx` — 신규 컴포넌트 (Variant C 디자인, 탭 핸들러)
- [ ] `src/index.css` — `@keyframes shake` 추가
- [ ] `GamePage.tsx` — RESULT 자동전환 useEffect 제거, shake CSS 적용, GameOverOverlay 렌더링
...
- [ ] `src/components/game/GameOverOverlay.tsx` — `position: 'fixed'` → `position: 'absolute'` 변경, 패널 상단에 핸들바·경고 아이콘·"GAME OVER" 타이틀 추가
- [ ] `src/pages/GamePage.tsx` — 루트 div style에 `position: 'relative'` 추가
...
- [ ] `src/pages/GamePage.tsx` — 타이틀 div style에 `position: 'relative', zIndex: 201` 추가
- [ ] `src/pages/GamePage.tsx` — HUD strip div style에 `position: 'relative', zIndex: 201` 추가
