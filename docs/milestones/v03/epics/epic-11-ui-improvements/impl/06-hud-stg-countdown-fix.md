...
### 수정: `src/pages/GamePage.tsx` L242
...
- **Breaking Change 없음**: HUD 스트립 외부 인터페이스 변경 없음. `StageArea` 컴포넌트 변경 없음.
- **DB 영향도**: 없음. UI 전용 수정.
...
- **리트라이 경로**: `retryGame()` 호출 시 store의 `stage`가 `resetGame()` 내부에서 초기화되기
  전에 `countdown`이 설정된다. 이 수정으로 리트라이 시 직전 stage 잔존 버그도 동시에 해결.
...
| # | 항목 | 유형 |
|---|---|---|
| AC1 | 게임 시작 카운트다운(3→2→1) 2250ms 동안 HUD STG 셀이 `--` 표시 | MANUAL |
| AC2 | 스테이지 N 진행 후 리트라이 시 카운트다운 중 직전 stage 값 미노출 | MANUAL |
| AC3 | 카운트다운 종료 후 게임 진행 중 STG 셀이 올바른 stage 값 표시 | MANUAL |
| AC4 | SHOWING / INPUT / RESULT 상태에서 STG 셀 회귀 없음 | MANUAL |
