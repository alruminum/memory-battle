...
## Story 12: 콤보 브레이크 쉐이크 애니메이션 (#98)

> impl: `impl/18-combo-break-shake.md`
> 관련 이슈: [#98](https://github.com/alruminum/memory-battle/issues/98)

콤보 타이머 만료(풀콤보 실패) 시 `ComboIndicator`에 variant-A 쇼크 웨이브 시각 효과 추가.
컨테이너 쉐이크(±8px×6, 0.4s) + 5개 블록 `scaleY(0)` 붕괴 + 배율 숫자 red flash 후 재등장.
100% CSS @keyframes, 외부 라이브러리 없음.

- [ ] `src/index.css` — `@keyframes comboShake`, `@keyframes blockBreak`, `@keyframes multBreak` + `.combo-breaking`, `.combo-mult-break` 클래스 추가
- [ ] `src/components/game/ComboIndicator.tsx` — `isBreaking` prop 추가, `animKey` + `prevBreaking` ref, CSS 클래스 조건부 적용, 블록 인라인 animation 분기
- [ ] `src/hooks/useGameEngine.ts` — `isFullCombo` 구조분해 추가, `comboBreaking` state + ref 관리, 반환값 확장
- [ ] `src/pages/GamePage.tsx` — `comboBreaking` 구조분해, `ComboIndicator`에 `isBreaking={comboBreaking}` 전달
