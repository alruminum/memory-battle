...
describe('GameOverOverlay — 이벤트 핸들러', () => {
  it('pointerdown 이벤트 발생 시 onConfirm이 호출된다 (wrong reason)', () => {
    ...
    const { container } = render(
      <GameOverOverlay reason="wrong" coinBalance={10} revivalUsed={false} onConfirm={onConfirm} />
    )
  ...
  it('pointerdown 이벤트 발생 시 onConfirm이 호출된다 (timeout reason)', () => {
    const onConfirm = vi.fn()
    const { container } = render(
      <GameOverOverlay reason="timeout" coinBalance={10} revivalUsed={false} onConfirm={onConfirm} />
    )
    const backdrop = container.firstChild as HTMLElement
    fireEvent.pointerDown(backdrop)
    expect(onConfirm).toHaveBeenCalledTimes(1)
  ...
  it('backdrop이 gameover-backdrop 클래스를 갖는다', () => {
    const { container } = render(
      <GameOverOverlay reason="wrong" coinBalance={10} revivalUsed={false} onConfirm={vi.fn()} />
    )
    const backdrop = container.firstChild as HTMLElement
